...
${Array.from(zodSchemasWithMissingDependencies).join("\n")}`
  ...
  // Check which specific function helpers are needed
  const hasSyncFunctions = Array.from(statements.values()).some(
    ({ typeName }) =>
      functionTypeMap.get(typeName) && !asyncFunctionTypeMap.get(typeName)
  );
  const hasAsyncFunctions = Array.from(statements.values()).some(
    ({ typeName }) => asyncFunctionTypeMap.get(typeName)
  );
...
`
  ...
  const functionHelpers =
    hasSyncFunctions || hasAsyncFunctions
      ? "\n" +
        [createFunctionSchemaHelper, createAsyncFunctionSchemaHelper]
          .filter(Boolean)
          .join("\n\n") +
        "\n"
      : "";
...
${
  zodImportToOutput.length
    ...
    originalImportsToOutput.length
      ...
      : ""
  }${functionHelpers}${Array.from(statements.values())
