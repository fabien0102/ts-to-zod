...
describe("generate", () => {
  ...
  describe("with @strict tag", () => {
    it("should generate strictObject schemas", () => {
      const sourceText = `
      /**
       * @strict
       */
  ...
  describe("with input/output mappings to manage imports", () => {
    ...
    describe("nameFilter imports", () => {
      const input = "./hero";
      const output = "./hero.zod";
      const inputOutputMappings: InputOutputMapping[] = [{ input, output }];
      const nameFilter = (name: string) => !name.startsWith("Alt");

      const heroVillainSourceText = `
      export interface Hero {
        name: string;
        powers: string[];
      }
      ...
      export interface AltHero {
        name: string;
        powers: string[];
      }

      export interface AltVillain {
        name: string;
        evilPlan: string;
      }	
      `;

      const heroVillainSchemaFile = generate({
        sourceText: heroVillainSourceText,
        nameFilter,
      });

      const personSourceText = `
      import { Hero, Villain, AltHero, AltVillain } from "${input}"
      ...
      const personSchemaFile = generate({
        sourceText: personSourceText,
        inputOutputMappings,
        nameFilter,
      });

      it("should filter out 'Alt' types", () => {
        expect(heroVillainSchemaFile.getZodSchemasFile(input))
          .toMatchInlineSnapshot(`
        "// Generated by ts-to-zod
        import { z } from "zod";
      ...
      it("should filter out 'Alt' imports", () => {
        expect(personSchemaFile.getZodSchemasFile(input))
          .toMatchInlineSnapshot(`
        "// Generated by ts-to-zod
        import { z } from "zod";
